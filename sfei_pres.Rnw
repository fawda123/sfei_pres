...
\title[Understanding nutrient pollution]{\textbf{Quantitative approaches to understanding nutrient pollution: Examples from the upper San Francisco Estuary}}
...
toplo2 <- select(toplo, date, fit0.5) %>% 
  na.omit()

p2 <- ggplot(toplo2, aes(x = date, y = exp(fit0.5))) + 
  geom_line() + 
  mytheme + 
  scale_y_continuous(ylab, limits = c(0, 4)) +
  scale_x_date(limits = xlims) 

pdf('fig/c10obspred.pdf', height = 2.5, width = 12, family = 'serif')
p1
p2
dev.off()

# din prdnrmplot
p3 <- prdnrmplot(toplo, logspace = F, col_vec = col_vec) + 
  mytheme + 
  scale_y_continuous(ylab) +
  theme(
    legend.position = 'top'
  ...
  scale_x_date(limits = xlims) 
pdf('fig/c10prdnrm.pdf', height = 3, width = 12, family = 'serif')
p3
dev.off()
...
p4 <- ggplot(toplo2, aes(x = date, y = flodiff)) + 
...
p5 <- ggplot(toplo3, aes(x = date, y = flo)) + 
  geom_line() +
  stat_smooth(se = F) + 
  mytheme + 
  scale_y_continuous(ylab) +
...
maxWidth = grid::unit.pmax(pD$widths[2:3], pE$widths[2:3])

pD$widths[2:3] <- maxWidth
pE$widths[2:3] <- maxWidth

pdf('fig/c10flow.pdf', height = 2.5, width = 12, family = 'serif')
grid.arrange(pD)
grid.arrange(pE)
dev.off()
@
...
pdf('fig/c10grd1.pdf', height = 2.4, width = 9, family = 'serif')
grid.arrange(pA)
dev.off()
pdf('fig/c10grd2.pdf', height = 2, width = 9, family = 'serif')
grid.arrange(pB)
dev.off()
@
